...
int main(int argc, char* argv[]) {
    ...
    // 读取配置文件
    CConfigFileReader config_file(str_chat_room_conf);
    ...
    for (size_t i = 0; i < GetRoomIdsSize(); i++)
    {
        pubSubService.AddRoomTopic(GetRoomId(i), 1); //默认先使用user id作为创建者
    }
